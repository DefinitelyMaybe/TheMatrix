...
// All inputs
const yearlyInterestRateInput = document.querySelector(
  "input#interestRate",
)! as HTMLInputElement;
const interestPerInput = document.querySelector(
  "select#peroid",
)! as HTMLSelectElement;
const initialAmountInput = document.querySelector(
  "input#initialAmount",
)! as HTMLInputElement;
const extraContributionsInput = document.querySelector(
  "input#extraContributions",
)! as HTMLInputElement;
const extraAmountInput = document.querySelector(
  "input#extraAmount",
)! as HTMLInputElement;
const totalYearsInput = document.querySelector(
  "input#term",
)! as HTMLInputElement;
...
function compute() {
  ...
  if (!changeFormula) {
    newValue = initialAmount *
      (1 + incuredInterestRate) ** totalIncuredInterestEvents;
  } else {
    const twoT = (1 + incuredInterestRate) **
      (interestCalculationsPerYear * totalIncuredInterestEvents);
    newValue = initialAmount * twoT +
      extra * ((twoT - 1) / (incuredInterestRate)) * (1 + incuredInterestRate);
  }
